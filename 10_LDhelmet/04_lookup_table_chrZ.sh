...
realpath $SAMPLE_DIR/chrZ*.snps > ldhelmet_input_chrZ.txt

#theta set based on 2015 finch paper nucleotide diversity

ldhelmet find_confs --num_threads 8 -w 50 -o parv_chrZ.conf $(<ldhelmet_input_chrZ.txt)
echo "find_confs done"
ldhelmet table_gen --num_threads 8 -c parv_chrZ.conf -t 0.00063 -r 0.0 0.1 10.0 1.0 100.0 -o parv_chrZ.lk
echo "lk tabls done"

ldhelmet pade --num_threads 8 -c parv_chrZ.conf -t 0.00063 -x 11 -o parv_chrZ.pade
echo "pade done"
#sbatch -p devel -t 00:10:00 ~/afc/10_LDhelmet/04_lookup_table_chrZ.sh
