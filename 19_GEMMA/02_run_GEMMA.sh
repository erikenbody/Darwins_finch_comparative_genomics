...
#bcftools view -t ^chrZ $VCF | bcftools annotate --set-id '%CHROM\_%POS\_%REF\_%FIRST_ALT' -O z -o ${CHR}_fulldata_merge_ID.vcf.gz
...
#qctool -g ${CHR}_fulldata_merge_ID.vcf.gz -ofiletype bimbam_dosage -og ${OUTNAME}/${CHR}_${OUTNAME}.geno

mkdir -p ${OUTNAME}/${CHR}_${OUTNAME}_gemma_out

$GEMMA/gemma -p $PHENO \
  -a ${CHR}_fulldata_merge_map.txt -g ${OUTNAME}/${CHR}_${OUTNAME}.geno \
  -lm 1 -outdir ${OUTNAME}/${CHR}_${OUTNAME}_gemma_out \
  -n 2 \
  -o ${CHR}_${OUTNAME}_species_lm_V2
